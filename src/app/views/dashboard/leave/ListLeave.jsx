  ...
  const fetchSalary = async () => {
    try {
      let data = await axios.get(
        "http://localhost:8091/leave/leave/findAll"
      );
      const salaries = data.data
      data = await axios.get(
        "http://localhost:8093/leave/employee/all"
      );
                  ...
                  <TableRow key={i}>
                    ...
                    <TableCell align="center">
                      <CheckCircleOutline style={{ color: "green", cursor: "pointer" }}  onClick={async () => {
                          await axios.put(
                            `http://localhost:8091/leave/leave/approve/${e.id}`
                          );
                          fetchSalary();
                        }} />
                      <DoDisturbOn   style={{ color: "red", cursor: "pointer" }}  onClick={async () => {
                          await axios.put(
                            `http://localhost:8091/leave/leave/decline/${e.id}`
                          );
                          fetchSalary();
                        }}/>
                    </TableCell>
                    <TableCell align="right">
                      <DeleteForeverIcon
                        ...
                        onClick={async () => {
                          await axios.delete(
                            `http://localhost:8091/leave/leave/delete/${e.id}`
                          );
                          fetchSalary();
                        }}
                      />
