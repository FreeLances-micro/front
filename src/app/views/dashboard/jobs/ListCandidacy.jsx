    ...
    const fetchCandidacies = async () => {
        try {
            const data = await axios.get(
                `http://localhost:8099/job/api/Candidacy/findByJob/${location.state.id}`
            );
            console.log(data);
                                    ...
                                    <TableRow key={i}>
                                        ...
                                        <TableCell align="center">
                                            <Switch
                                                ...
                                                onChange={async (event) => {console.log(event.target.checked);
                                                    if (event.target.checked === true){
                                                        await axios({
                                                            method: "put",
                                                            url: `http://localhost:8099/job/api/Candidacy/acceptCandidacy/${ee.id}`,
                                                        }).then((response)=>{
                                        ...
                                        <TableCell align="center">
                                            <PictureAsPdfIcon
                                                ...
                                                onClick={async (e) => {
                                                    e.preventDefault();
                                                    console.log(ee.id);
                                                    window.open(`http://localhost:8099/job/api/Candidacy/downloadResume/${ee.id}`,"_blank");
                                                }}
                                            />
                                        </TableCell>
                                    </TableRow>
